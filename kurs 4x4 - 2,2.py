...
def phi2_(x, y, z):
    #if math.sqrt(x ** 2 + y ** 2 + (z - z0) ** 2) > 0.35:
    return 2 * (math.exp(-a1 * (x - x0) ** 2) * math.exp(-b1 * y ** 2) * math.exp(-c1 * z ** 2))/math.sqrt(x**2+y**2+(z-z0)**2) - l*(x**2+y**2+z**2) * (math.exp(-a1 * (x - x0) ** 2) * math.exp(-b1 * y ** 2) * math.exp(-c1 * z ** 2))/2
    #else:
    #    return 0.0
def phi21_(x, y, z):
    #if math.sqrt(x ** 2 + y ** 2 + (z - z0) ** 2) > 0.35:
...
def phi2_r(r, teta, phi):
    ...
    y = r*math.sin(teta)*math.cos(phi)
    z = r*math.cos(teta)
    return math.exp(-a1*(x-x0)**2)*math.exp(-b1*y**2)*math.exp(-c1*z**2)*J(r, teta)
...
def phi2_r_(r, teta, phi):
    ...
    y = r*math.sin(teta)*math.cos(phi)
    z = r*math.cos(teta)+ z0
    return 2 * (math.exp(-a1 * (x - x0) ** 2) * math.exp(-b1 * y ** 2) * math.exp(-c1 * z ** 2)) / 0.05 - l * (
                x ** 2 + y ** 2 + z ** 2) * (
                math.exp(-a1 * (x - x0) ** 2) * math.exp(-b1 * y ** 2) * math.exp(-c1 * z ** 2)) / 2
